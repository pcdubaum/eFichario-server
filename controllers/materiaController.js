...
exports.getMateriaConcurso = cathAsync(async (req, res, next) => {

  const tema = req.params.tema;
  const concurso = req.params.concurso;

  const materias = await Materia.find({ tema: { $regex: tema } });   

  if (!materias) { new AppError('Não foi possível encontrar o recursos', 404) };
